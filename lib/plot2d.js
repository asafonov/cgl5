...
function drawAxis(canvas_id, zero_point) {
	...
	ctx.lineTo(settings.legend_border-2, cvs.height-settings.legend_border+2);
	ctx.lineTo(cvs.width, cvs.height-settings.legend_border+2);
	ctx.stroke();
	ctx.closePath()
	if (zero_point>0) {
	    ctx.beginPath();
...
function drawValues(canvas_id, data, zero_point) {
    ...
    ctx.strokeText(Math.round(min_value*100)/100, 1, cvs.height-settings.legend_border);
    if (zero_point>0) {
